...
def takeTurn(buttonIn, cordA, cordB, isPlay):
    ...
    global Xgoing
    if isPlay:
        if Xgoing and buttonIn["text"] == "[ ]":
            buttonIn.config(text="[X]")
            turnLabel.config(text="O's Turn")
            Xgoing = False
...
btnPlay = Button(root, text="Reset", command=lambda: Restart(
    btnList), bg="light blue", highlightbackground="light blue", highlightcolor="light blue")
