...
root = Tk()
root.title("The Student Grouper")
num = 0
...
list_2 = Listbox(root, height=4, width=40)
...
def randomize(listIn,listOut,amount):
    ...
    tempList = []
    tempList.delete(0,END)
    for lp in range(num):
        tempList.append(listIn.get(lp))
    for lp in range(len(tempList)):
        listOut.insert(lp,tempList[lp] + f" is in group {randint(0,amount)}")
    print (tempList)
...
sub_btn4 = Button(root,text ="Group students",command=lambda: randomize(listbox,list_2,int(ginVar.get())))
...
exitButton = Button(root,text = "Save and quit",command=lambda: quitList(listbox,list_2))
...
listbox.grid(row=10, column=0)
list_2.grid(row=11,column=0)
